...
- Can add CORS (that let you hit on your API from any domain) for every domain or only a few
...
- Currently, if an error occured in the full process, the `api.php` file will be part filled, and the closing `Route::group` statement could not be written in this file, leading to some errors if trying to re-run the command again. An effort should be made to undo every changes if something occured.
- Thinking of removing `--fake` feature as it causes a lot of issues when a table is composed of foreign keys, and would be a lot to do for the first version to make this feature available (maybe in a second version ?).
